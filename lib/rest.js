...
Rest_doc_api.prototype.startServer = function(config){
  ...
  const ws_options = { 
    allowEIO3: true,
    cors: {
      origin: [self.config_server.origins],
      credentials: true,
    }
  };
...
Rest_doc_api.prototype.middlewareAccess = function(req, res, next){ 

  var api_key = req.header('x-api-key')

  // console.log('API_KEY:', Config_API.API_KEY);
  // console.log('Request URL:', req.originalUrl);
  // // console.log('Host:', req.headers.origin);
  // console.log('Header:', api_key);
  
  if(Config_API.API_KEY != null && api_key != Config_API.API_KEY){
    var msg = 'Your API_KEY' + ((typeof api_key == 'undefined') ? ' is not defined in the request header.': '=' + api_key + ' is wrong.');
    res.status(401).json({error : "Access denied.", detail: msg});
  }
  else {
    next();
  }
...
Rest_doc_api.prototype.buildKnexFilter = function(knex, filter, logic) {
  ...
  if(Array.isArray(filter.filters)){
    ...
    switch(f.op){
      case "EQUAL STRICT":  knex.whereRaw('`' + f.field + '` = ? COLLATE utf8_bin', [f.value]);  break;
...
Rest_doc_api.prototype.getFilter = function(filter){
  ...
  switch(v.operator){
    ...
    case 'eq_strict':       return {field: v.field, op: "EQUAL STRICT", value: v.value};          break; //ok    
...
Rest_doc_api.prototype.tbMethodGetAll = function(config, primary, columns, hidden) {
  ...
  self.app.get(url, this.middlewareAccess, function(req, res){      
...
Rest_doc_api.prototype.tbMethodSearch = function(config, primary, columns, hidden) {
  ...
  self.app.post(url, this.middlewareAccess, function(req, res){
...
Rest_doc_api.prototype.tbMethodSearchColumn = function(config, primary, columns, hidden) {
  ...
  self.app.post(url, this.middlewareAccess, function(req, res){
    ...
    if(typeof req.body.field === 'undefined' || typeof req.body.operator === 'undefined') {
      ...
      var table = (config.view === null) ? config.table : config.view;      
      self.knex(table).distinct(req.body.field).where(params).andWhere(function(){
          //self.processFilter(this, req.body.filter)        
...
Rest_doc_api.prototype.tbMethodPut = function(config, primary, columns) { 
  ...
  self.app.put(url, this.middlewareAccess, function(req, res){ 
...
Rest_doc_api.prototype.tbMethodPatch = function(config, primary, columns) { 
  ...
  self.app.patch(url, this.middlewareAccess, function(req, res){ 
...
Rest_doc_api.prototype.tbMethodDelete = function(config, primary, columns) {
  ...
  self.app.delete(url, this.middlewareAccess, function(req, res){
...
Rest_doc_api.prototype.spMethodPost = function(config) {
  ...
  self.app.post(url, this.middlewareAccess, function(req, res){
    ...
    self.knex.raw(`call ${config.procedure}(${Object.values(req.body).join(',')});`)
      .then(function(rows) {
        // console.log(rows)
        res.status(200).jsonp({total: rows[0][0].length, data: rows[0][0]});   
      })        
      .catch(function(err){
        console.log(err);
        res.status(500).json({error : "Error executing Query ", detail: err});
