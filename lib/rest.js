...
Rest_doc_api.prototype.buildRoutes = function(api){    
    ...
    self.routes = Config_API.ROUTE_CONFIG;
    if(typeof api.routes !== 'undefined')
      self.routes = extend({}, Config_API.ROUTE_CONFIG, api.routes);

    // console.log('ROUTES', self.routes);
    api.routes.tb.forEach(function(v, i){

      self.buildTB(v);

    })

    api.routes.fn.forEach(function(v){

      self.buildFN(v);

    })

    api.routes.sp.forEach(function(v){
