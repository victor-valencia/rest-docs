...
// function Rest_doc_api(router, con, sockets){

//   var self = this;
//   self.app = router;

//   self.sockets_ = sockets;    
...
Rest_doc_api.prototype.startServer = async function(){
  ...
  if( typeof self.config != 'undefined') {
    ...
    const pool = await db.getConnection(self.config.DATABASE);
    self.con_ = pool.con;
...
Rest_doc_api.prototype.printRoutes = function(){
  ...
  self.app._router.stack.forEach(function(v){
    ...
    if(typeof v.route != 'undefined'){
      ...
      routes.push({method: method[0].toUpperCase(), url: v.route.path, color: colors[method[0]]});
    }
  ...
  self.app.get(self.base + 'docs', function(req, res) {
    res.render('pages/index', {routes: routes});
